...
'''
    {
        ...
        "ch":false,
        "incr":false,
        "zSetEntries":[{"element":"eyJjdXN0b21lck5hbWUiOiJTYW0gVGVzdCIsImVtYWlsIjoic2FtLnRlc3RAdGVzdC5jb20iLCJwaG9uZSI6IjgwMTU1NTEyMTIiLCJiaXJ0aERheSI6IjIwMDEtMDEtMDMifQ==","score":0.0}],
        "zsetEntries":[{"element":"eyJjdXN0b21lck5hbWUiOiJTYW0gVGVzdCIsImVtYWlsIjoic2FtLnRlc3RAdGVzdC5jb20iLCJwaG9uZSI6IjgwMTU1NTEyMTIiLCJiaXJ0aERheSI6IjIwMDEtMDEtMDMifQ==","score":0.0}]
    }
'''
...
'''
    {"customer":"Sam Test","score": -10.0, "email":"sam.test@test.com","birthDay":"1960-01-01"}
'''
...
'''
    {"customer":"Larry.Abram@test.com","score":33.5,"riskDate":"2025-01-08T08:07:35.043Z"}
'''
kafkaCustomerRiskSchema = StructType(
    [
        StructField("customer", StringType()),
        StructField("score", StringType()),
...
spark = SparkSession.builder.appName("stediStream").getOrCreate()
...
redisDF.withColumn("value", from_json("value", kafkaRedisSchema))\
...
decodedDF.withColumn("customer", from_json("customer", kafkaJsonSchema))\
    .select(col("customer.*"))\
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
eventsDF = eventsDF.selectExpr("cast(value as string) value")
...
eventsDF.withColumn("value", from_json("value", kafkaCustomerRiskSchema))\
    .select(col("value.*"))\
...
joinedDFs = customerRiskDF.join(emailAndBirthYearStreamingDF, expr("customer = email"))
...
#joinedDFs.writeStream.outputMode("append").format("console").start().awaitTermination()

joinedDFs.selectExpr("cast(customer as string) as key",
