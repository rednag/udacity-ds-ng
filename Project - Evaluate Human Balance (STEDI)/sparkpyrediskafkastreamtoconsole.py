...
spark = SparkSession.builder.appName("redisKafkaStreaming").getOrCreate()
...
redisDF = redisDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
redisDF.withColumn("value", from_json("value", kafkaRedisSchema))\
    .select(col('value.key'),col('value.existType'), col('value.ch'),\
